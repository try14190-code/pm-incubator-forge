export const VariableAndDataTypes = () => {
    ...
    // 평균을 계산할 것인데 let z <- 이름이 이와 같으면 z가 뭔지 알 수 있을까? 없을까?
    // 명시성이 없기 때문에 z라는 변수가 무엇을 위한 정보인지 알 수 없음
    // 이 문제를 해결하려면 명시성을 주변 해결됨
    // 고로 let average 혹은 mean <- 형태로 작성하면 해결됨
    // 실제로 좀 옛날 회사나 꽤 많은 이상한 회사는
    // 아래와 같은 변수명을 볼 수 있음
    // let fsdfh = 100; <- 모든 코드가 이런 형태로 되어 있음
    // 보는 순간 멘탈 터짐
    // export const VADT = () => {꿍시렁 꿍시렁}
    // VADT가 뭐지??
    ...
    // first, seocnd, third, fourth, fifth, sixth
    // 일일히 변수 이름을 지정하라고 하면 너무 피곤함
    ...
    return (
        <div>
            ...
            {/* pre 태그 사이에 있는 내용들은 전체가 웹 페이지 그대로 반영됨 */}
            <pre>{
                `
...
let fifthUndefined = ${fifthUndefined}              // Undefiend 타입
